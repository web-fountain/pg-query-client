...
// AIDEV-NOTE: Queries
export async function getItem(id: string): Promise<NodePayload> {
  const url = `/api/fs/item?id=${encodeURIComponent(id)}`;
  return safeFetchJSON<NodePayload>(url, { cache: 'no-store' });
}

export async function getChildrenIds(id: string): Promise<string[]> {
  const url = `/api/fs/children?id=${encodeURIComponent(id)}`;
  return safeFetchJSON<string[]>(url, { cache: 'no-store' });
}

export type ChildWithData = { id: string; data: NodePayload };
